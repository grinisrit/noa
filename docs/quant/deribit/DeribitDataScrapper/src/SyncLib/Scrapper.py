...
async def call_api(msg, URL_TO_SCRAP):
...
async def send_request(msg, URL_TO_SCRAP, show_answer=False) -> json:
    if URL_TO_SCRAP["test_net"]:
        url = 'wss://test.deribit.com/ws/api/v2'
    else:
        url = 'wss://www.deribit.com/ws/api/v2'
    res = await call_api(json.dumps(msg), url)
