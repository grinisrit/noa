...
{
    const int nkin = DCSData::get_kinetic_energies().numel();
    auto fCM = torch::zeros({nkin, 2}, tensor_ops);
    auto screen = torch::zeros({nkin, 9}, tensor_ops);
    auto fspin = torch::zeros_like(DCSData::get_kinetic_energies());
    auto invlambda = torch::zeros_like(DCSData::get_kinetic_energies());
    ...
    default_coulomb_transport(
        G, screen, fspin, torch::tensor(1.0, tensor_ops));
